...
`COLS`, `ROWS`, `BLOCK`, `COLORS`, `LINE_SCORES`, `dropInterval`, `QUEUE_SIZE`, `ENERGY_MAX`, `ENERGY_GAIN`, `SLOW_DURATION`, `SLOW_FACTOR`. If `COLS`/`ROWS`/`BLOCK` change, update the `#board` canvas `width`/`height` in `index.html` to match (`COLS × BLOCK`, `ROWS × BLOCK`).
