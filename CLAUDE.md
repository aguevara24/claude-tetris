# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project

Classic Tetris implemented in vanilla JavaScript (HTML5 Canvas, CSS3). No dependencies, no build step, no `package.json`. Three files: `index.html`, `style.css`, `game.js`.

## Running

No build/install step. Open directly or serve statically:

```bash
start index.html          # Windows, open directly
python3 -m http.server 8000
npx serve .
php -S localhost:8000
```

There is no test suite, linter, or bundler configured in this repo.

## Architecture

All game logic lives in `game.js` as top-level mutable state (`board`, `current`, `nextQueue`, `score`, `lines`, `level`, `combo`, `paused`, `gameOver`, `dropInterval`, plus the skill-system state below) plus functions operating on that shared state — no classes, no modules.

- **Board model**: `ROWS × COLS` matrix; each cell is `0` (empty) or a color index `1–8` identifying which piece locked there (`1–7` tetrominoes, `8` the N/tuerca piece).
- **Pieces**: defined as square matrices in `PIECES`. Rotation is done via `rotateCW` (transpose + reverse), not by storing pre-rotated states.
- **Collision** (`collide`): checks board bounds and overlap with locked cells.
- **Wall kicks** (`tryRotate`): after rotating, tries offsets `[0, -1, 1, -2, 2]` columns until a non-colliding position is found, else the rotation is discarded.
- **Game loop** (`loop`): driven by `requestAnimationFrame`; accumulates elapsed time in `dropAccum` and advances the piece one row once `effectiveInterval()` (see Habilidades) is exceeded.
- **Line clearing** (`clearLines`): scans bottom-to-top, splices full rows out and unshifts empty rows at the top.
- **Scoring**: `LINE_SCORES = [0, 100, 300, 500, 800]` multiplied by `level`; hard drop adds 2 pts/row dropped, soft drop adds 1 pt/row.
- **Leveling**: level = `floor(lines / 10) + 1`; `dropInterval = max(100, 1000 - (level - 1) * 90)` ms.
- **Ghost piece** (`ghostY`): projects the current piece straight down to its landing row, drawn at `globalAlpha = 0.2`.
- **Rendering**: `draw()` clears and redraws the grid, locked board, ghost piece, and current piece every frame onto `#board`; `drawNext()`/`drawQueue()`/`drawHold()` render piece previews via the shared `drawPiecePreview()` helper, which centers a shape of any size in a `gridSpan × gridSpan` cell grid using its bounding box (`pieceBounds`).

Flow: `init()` builds an empty board, seeds `nextQueue` with `QUEUE_SIZE` pieces, calls `spawn()` (which shifts a piece off `nextQueue` into `current` and pushes a new random one), then starts the `requestAnimationFrame` loop. `spawn()` triggers `endGame()` and returns early if the newly spawned piece immediately collides.

Input is handled by a single `keydown` listener (arrow keys move/rotate/soft-drop, Space hard-drops, X also rotates, P toggles pause, E opens the skill menu, C holds); it's a no-op while paused or game-over except for the pause key itself. While the skill menu is open, the same listener is redirected to menu navigation (arrows/Enter/digits/Escape) instead.

## Habilidades (skill system)

A single-charge energy bar fills as lines are cleared and, once full, lets the player spend it on one of five skills via an in-game menu.

- **Energy**: `energy` (0–`ENERGY_MAX`) gains `ENERGY_GAIN[min(cleared, 4)]` per lock in `clearLines()` — 10/25/45/70% for 1/2/3/4+ simultaneous lines (the N piece can clear more than 4 rows at once, hence the clamp). The bar and `Pulsa E` hint are driven by `updateHUD()`.
- **Menu** (`openSkillMenu`/`closeSkillMenu`/`chooseSkill`, `#skill-overlay`): `E` freezes the game loop (`freeze()`) and opens the menu, only when `energy >= ENERGY_MAX`. Navigate with arrows/digits 1–5/Enter/click; `Esc` cancels without spending energy. Selecting a valid skill runs its `apply()` from the `SKILLS` array, resets `energy` to 0, and calls `resume()`.
- **`freeze()`/`resume()`**: shared helpers that cancel/restart the `requestAnimationFrame` loop, used by both `togglePause()` and the skill menu so pausing and the menu never conflict or stack loops.
- **Ver 5 siguientes**: sets `previewExpanded = true` permanently, revealing the `#queue-section` panel (`drawQueue()`, piece indices `nextQueue[1..4]`) alongside the always-visible `NEXT` (`nextQueue[0]`).
- **Intercambiar pieza** (`swapWithQueue`): swaps `current` with `nextQueue[0]`, resetting both to their spawn shape/position via `resetPiece()`; aborts (no energy spent) if the incoming piece would collide (`canSwap()`).
- **Ralentizar 10s**: sets `slowRemaining = SLOW_DURATION`; `loop()` counts it down each frame and `effectiveInterval()` multiplies `dropInterval` by `SLOW_FACTOR` while it's active. `#slow-indicator` shows the remaining time.
- **Deshacer colocación** (`undo`): `snapshot()` captures board/piece/queue/hold/score/lines/level/dropInterval right before every `merge()` in `lockPiece()`; `undo()` restores that snapshot. Disabled (`undoSnapshot === null`) before the first piece locks.
- **Reservar (Hold)** (`doHold`, key `C`): first use sets `holdUnlocked = true` permanently and reveals the `#hold-section`/`C` control hint; each subsequent press stores/swaps `current` with `holdPiece` (via `resetPiece()`), limited to one use per piece — `holdUsed` is set in `doHold()` and cleared in `lockPiece()`, not in `spawn()`, so a piece pulled from the queue by a hold can't be hold-swapped again until it locks.

## Tabla de records local y pantalla de inicio

The game no longer starts running the instant the page loads: `init()` only resets state (board, piece, HUD) and draws one frame; the `requestAnimationFrame` loop is started separately by `startGame()`, called by the **Jugar** button or by pressing `Enter`/`Space` while `#start-overlay` is visible. `newGame()` is the `init(); startGame();` combo used by both the Jugar button and the `Reiniciar` button (game over goes straight back into a fresh game rather than back to the start screen).

- **Combo counter**: `combo` (top-level state, reset in `init()`) increments in `clearLines()` whenever a lock clears at least one line, and resets to `0` the moment a lock clears zero lines. `bestCombo` tracks the highest `combo` reached in the current run (`bestCombo = Math.max(bestCombo, combo)`), reset to `0` in `init()`. Both are included in the undo skill's `snapshot()`/`undo()` (game.js `snapshot()`/`undo()`) so reverting a placement also reverts `combo` consistently with the restored board — `bestCombo` is a monotonic running max and is intentionally left alone by undo.
- **Persistence** (`localStorage` key `'tetris-scores'`, mirroring the `THEME_KEY` pattern): stores `{ top: [{name, score, lines, level, date}, …] (max 5, sorted desc by score), bestCombo, maxLines }`. `loadScores()`/`saveScores()` wrap `JSON.parse`/`localStorage.setItem` in `try/catch` so corrupted or unavailable storage falls back to an empty structure instead of breaking page load. `qualifies(score)` reports whether a score would land in the top 5 (strictly beats the current 5th place, or the table isn't full yet). `addScore(name, score, lines, level)` inserts, sorts, truncates to 5, and updates `bestCombo`/`maxLines`. `recordRunStats()` updates just `bestCombo`/`maxLines` for every run, whether or not it made the top 5. The last-used player name is remembered separately under `localStorage` key `'tetris-last-name'`.
- **Shared rendering**: `renderScoreTable(container, highlightIndex)` builds the top-5 `<table>` markup (escaping the player name via `escapeHtml()`) into any container; used by both `renderStartScreen()` (populates `#start-table` plus the best-combo/max-lines stats) and the game-over flow (`#gameover-table`). Row `highlightIndex` gets the `.highlight` class for the score just saved.
- **Start screen** (`#start-overlay`, same `.overlay` pattern as `#overlay`/`#skill-overlay`, visible by default on page load): shows the top-5 table plus best combo / max lines, a **Jugar** button, and a **Resetear records** button (`resetScores()`, gated behind `confirm()`, wipes the stored data and re-renders).
- **Game over integration**: `endGame()` calls `recordRunStats()` unconditionally, then checks `qualifies(score)` — if true it shows `#name-entry` (`#score-name` input, defaulted from `'tetris-last-name'`, plus a **Guardar** button wired to `saveCurrentScore()`) instead of the table; otherwise it renders `#gameover-table` directly. `saveCurrentScore()` calls `addScore()`, remembers the name, and re-renders the table with the new row highlighted. `togglePause()` hides both `#name-entry` and `#gameover-table` when showing the PAUSA overlay, since it reuses the same `#overlay` element.
- **Input passthrough**: the global `keydown` listener returns immediately for any `<input>` target except `#theme-toggle` (a checkbox, which must keep working with keyboard) — this lets the player type into `#score-name` without the game reacting to keystrokes. `#score-name` also gets `user-select: text` scoped in CSS since `body { user-select: none; }` is global.

## Tuning constants (in `game.js`)

`COLS`, `ROWS`, `BLOCK`, `COLORS`, `LINE_SCORES`, `dropInterval`, `QUEUE_SIZE`, `ENERGY_MAX`, `ENERGY_GAIN`, `SLOW_DURATION`, `SLOW_FACTOR`, `MAX_SCORES`. If `COLS`/`ROWS`/`BLOCK` change, update the `#board` canvas `width`/`height` in `index.html` to match (`COLS × BLOCK`, `ROWS × BLOCK`).
